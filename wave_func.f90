...
program hydrogen
  ...
  !
  open (7,file='wfc_numeric.out',status='unknown',form='formatted')
  open (8,file='wfc_analytic.out',status='unknown',form='formatted')
  !
...
1 continue
  ...
  do i=0,mesh  
     write (7,*) r(i), y(i)/sqr(i), (y(i)*sqr(i))*(y(i)*sqr(i)), e
  end do
  write (7,'(/)')
  
  write(8,"('#       r             R(r)          X(r)=rR(r)          e')")
  do i=0,mesh
     write (8,*) r(i), 2*(zeta/2)**(3/2)*(1-zeta/2*r(i))*exp(-zeta*r(i)/2) , (y(i)*sqr(i))*(y(i)*sqr(i)), e
  end do
  write (8,'(/)')
  go to 1
